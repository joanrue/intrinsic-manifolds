function [group_data,ind_data] = compute_group_pca(group_data,ind_data,opts,Nsub,tss)

group_data.PCA = zeros(tss*Nsub,opts.dimension);
ori_data = ind_data{1}.PCA(:,1:opts.dimension);
...
for i = 2:Nsub
    ...
    sub_data = ind_data{i}.PCA(:,1:opts.dimension);
...
for i = 1:Nsub
    ind_data{i}.PCA = squeeze(new_data(i,:,:));
    group_data.PCA((i-1)*tss*length(stages)+1:i*tss*length(stages),:) = squeeze(new_data(i,:,:));
end
